...
```{r}
counties <- readRDS("08-shiny-apps/census-app/data/counties.rds")
head(counties)
```
...
library(maps)
library(mapproj)
source("08-shiny-apps/census-app/helpers.R")
counties <- readRDS("08-shiny-apps/census-app/data/counties.rds")
...
- The `08-shiny-apps/census-app` path specification isn't needed for Shiny
...
```{r eval=FALSE}
runApp("08-shiny-apps/census-app")
```

## New example - stock visualizations

```{r eval=FALSE}
runApp("08-shiny-apps/stockVis")
```
