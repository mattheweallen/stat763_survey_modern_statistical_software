...
student_data_ice_cream <- drop_na(student_data_filtered_48_to_96, height, Preference_1)
```
### Student Ice Cream Preferences and Heights Survey
...
p + geom_histogram(position="identity", colour="grey40", alpha=0.2, bins = 10) + 
  labs(title="Student Heights by Sex", 
       subtitle="data filtered to range 48 inches to 96 inches") +
    facet_grid(. ~ sex)
```
...
geom_bar(aes(fill=sex), width = 0.5) + 
  ...
  labs(title="Student Ice Cream Preferences by Sex", 
       subtitle="data filtered to range 48 inches to 96 inches") +
facet_grid(. ~ PreferenceNumber)
```

```{r}
#plot number one ice cream preference by height
p <- ggplot(student_data_ice_cream, aes(Preference_1, height))
p + geom_boxplot() + 
  labs(title="Student Height by Number One Ice Cream Preference", subtitle="data filtered to range 48 inches to 96 inches")
```
...
Row
-----------------------------------------------------------------------
### Data Source file name is `r data_file_name`. Original Data Set with `r num_students_height_not_converted` heights not converted.
