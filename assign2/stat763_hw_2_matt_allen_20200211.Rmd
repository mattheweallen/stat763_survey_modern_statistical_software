...
heights <- tibble(id = ids, reported_date = dates, sex = sex, 
                  height_text = str_squish(str_trim(heights_text)))
...
#number na before cleaning
sum(is.na(as.numeric(heights$height_text)))
...
clean_heights <- function(height_text) {
  ...
  #check for pattern like 5'11 inside string.
  #pattern_1 <- "\\d'\\s*\\d*"
  
  #split_pattern_1 <- "\\s*['.,*\s]+\\s*"
  split_pattern_1 <- "\\s*['\\s]+\\s*"
  pattern_1 <- paste0("\\d",split_pattern_1,"\\d*")
  if(any(str_detect(heights_text, pattern_1))) {
    ft_inches_which = str_which(height_text,pattern = pattern_1)
    pattern_to_split = heights$height_text[ft_inches_which]
    split_data = str_split(pattern_to_split,split_pattern_1, simplify = TRUE)
    #str_which(heights$height_text,pattern = "\\d'\\d+")
    #blarg1 <- str_split(pattern_to_split,"'", simplify = TRUE)
    
    #take the feet part multiply by 12 and add the inches part to it
    converted_heights[ft_inches_which] = as.numeric(split_data[,1])*12 +   as.numeric(str_extract(split_data[,2],"\\d*"))
  }
...
sum(is.na(cleaned_heigths_data))



heights$height_text[which(is.na(cleaned_heigths_data))]

testy <- tibble(height_text = heights$height_text, cleaned_heigths_data = cleaned_heigths_data)


#entered_as_feet = which(cleaned_heigths_data >= 5 & cleaned_heigths_data <= 7)
#entered_as_feet
#cleaned_heigths_data[entered_as_feet] = cleaned_heigths_data[entered_as_feet]*10
```
